		...
		if string.byte(text, pos + 1) == cX then
			...
			if s == nil then
				...
				if s == nil then
					...
					if s == nil then
						json._parseError("Failed to parse number", text, pos)
					end
				end
			end
		...
		if b == cBackslash then
			...
			elseif bp == cU then
				...
				if string.len(h) ~= 4 then
					json._parseError("Ran out of text when parsing unicode escape.", text, pos)
				end
				
				local num = tonumber(h, 16)
				
				if num == nil then
					json._parseError("Expected valid unicode hex literal.", text, pos)
				end
...
function json._parseTable(cfg, text, pos)
	...
	while true do
		...
		if not ok then
			json._parseError("Ran into error when reading key for table: " .. key, text, pos)
		end
		
		pos = json._eatWhitespace(text, pos)
		
		if string.byte(text, pos) ~= cEquals then
			json._parseError("Expected equals symbol after table key.", text, pos)
		end
